...
export class GameService {
  ...
  async getGame(
    ...
    const game = await this.gameModel
      .findOne({
        ...
        telegramMessageThreadId: messageId,
        isDeleted: false,
      })
      .populate('question')
      .populate('guesser')
  ...
  async advanceStage(
    ...
    if (!updatedGame) {
      throw new NotFoundException(
        `Game not found or already ended for chat ${telegramChatId} message ${telegramMessageThreadId}`,
      );
    }
  ...
  async checkAnswer(
    ...
    if (!game)
      throw new NotFoundException(
        `No active game found for chat ${telegramChatId} message ${telegramMessageThreadId}`,
      );
    ...
    this.logger.log(`comparing answers: "${nGiven}" vs "${nCorrect}"`);
    return nGiven === nCorrect;
  }

  // TODO consider timing as well, logarithmic decay?
  public getScoreFromStage(
    difficulty: number = 1,
    stage: GAME_STAGE,
    wrongAnswers: number,
  ): number {
  ...
  async endCurrentGame(
    ...
    await this.gameTimerService.stopTimer(
      telegramChatId,
      telegramMessageThreadId,
    );
  ...
  public async advanceGame(
    ...
    try {
      ...
      if (!game) {
        this.logger.log(
          `Game ${chatId}_${telegramMessageThreadId} not found, may have been ended already`,
        );
        return null;
      }
      ...
      this.logger.log(
        `Advancing game ${chatId}_${telegramMessageThreadId} to ${newStage}`,
      );
      ...
      this.logger.error(
        `Error advancing game ${chatId}_${telegramMessageThreadId}:`,
        error,
      );
      return null;
    }
  ...
  public async revealAnswer(
    ...
    if (!game) {
      this.logger.log(
        `Game ${chatId}_${telegramMessageThreadId} not found for revealing`,
      );
      return;
    }

    this.logger.log(
      `Revealed answer for game ${chatId}_${telegramMessageThreadId}`,
    );
    await this.telegramService.revealAnswer(
      chatId,
      telegramMessageThreadId,
      question,
    );
