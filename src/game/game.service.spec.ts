...
describe('GameService', () => {
  ...
  describe('checkAnswer()', () => {
    ...
    it('does not accept substrings as correct', async () => {
      ...
      const sub = await service.checkAnswer(
        makeGame('QUESTION'),
        'Q',
        chatId,
        threadId,
    ...
    it('strips surrounding quotes before comparing', async () => {
      const dq = await service.checkAnswer(
        makeGame('Paris'),
        '"Paris"',
        chatId,
        threadId,
      ...
      expect(dq).toBe(true);
      const sq = await service.checkAnswer(
        makeGame('Rome'),
        "'Rome'",
        chatId,
        threadId,
    ...
    it('removes trailing punctuation before comparing', async () => {
      const ex = await service.checkAnswer(
        makeGame('Tokyo'),
        'Tokyo!!!',
        chatId,
        threadId,
      ...
      expect(ex).toBe(true);
      const qu = await service.checkAnswer(
        makeGame('Berlin'),
        'Berlin?',
        chatId,
        threadId,
    ...
    it('treats Cyrillic ё and е as equal', async () => {
      ...
      const test1 = await service.checkAnswer(
        makeGame('ёлка'),
        'елка',
        chatId,
        threadId,
      ...
      const test2 = await service.checkAnswer(
        makeGame('елка'),
        'ёлка',
        chatId,
        threadId,
    ...
    it('correctly matches various colon-separated values', async () => {
      ...
      const test3 = await service.checkAnswer(
        makeGame('16:9'),
        '16 : 9',
        chatId,
        threadId,
    ...
    it('compares exact strings from production logs', async () => {
      ...
      console.log(
        'Correct answer char codes:',
        [...correctAnswer].map((c) => c.charCodeAt(0)),
      );
      console.log(
        'Given answer char codes:',
        [...givenAnswer].map((c) => c.charCodeAt(0)),
      );
    ...
    it('detects lookalike characters that could cause production issues', async () => {
      ...
      console.log(
        'Normal colon char codes:',
        [...normalColon].map((c) => c.charCodeAt(0)),
      );
      console.log(
        'Fullwidth colon char codes:',
        [...fullwidthColon].map((c) => c.charCodeAt(0)),
      );
      console.log(
        'Modifier colon char codes:',
        [...modifierLetterColon].map((c) => c.charCodeAt(0)),
      );
      console.log(
        'Ratio sign char codes:',
        [...ratioSign].map((c) => c.charCodeAt(0)),
      );

      // These should NOT match
      const test1 = await service.checkAnswer(
        makeGame(normalColon),
        fullwidthColon,
        chatId,
        threadId,
      );
    ...
    it('handles potential invisible characters and normalization', async () => {
      ...
      console.log(
        'Normal answer char codes:',
        [...normalAnswer].map((c) => c.charCodeAt(0)),
      );
      console.log(
        'With zero-width space char codes:',
        [...withZeroWidthSpace].map((c) => c.charCodeAt(0)),
      );
      console.log(
        'With zero-width non-breaker char codes:',
        [...withZeroWidthNonBreaker].map((c) => c.charCodeAt(0)),
      );

      // These should NOT match because they contain invisible characters
      const test1 = await service.checkAnswer(
        makeGame(normalAnswer),
        withZeroWidthSpace,
        chatId,
        threadId,
      );
    ...
    it('throws NotFoundException if no game provided', async () => {
      await expect(
        service.checkAnswer(null as any, 'x', chatId, threadId),
      ).rejects.toThrow('No active game found');
    });

    it('handles numeric answers with Cyrillic unit (e.g., 1500 м)', async () => {
      const correct = '1500 м'; // note: Cyrillic "м"

      // exact match should be true
      const exact = await service.checkAnswer(
        makeGame(correct),
        '1500 м',
        chatId,
        threadId,
      ...
      // trailing punctuation should be ignored
      const withDot = await service.checkAnswer(
        makeGame(correct),
        '1500 м.',
        chatId,
        threadId,
      ...
      // Latin 'm' should NOT match Cyrillic 'м'
      const latinM = await service.checkAnswer(
        makeGame(correct),
        '1500 m',
        chatId,
        threadId,
      );
      expect(latinM).toBe(false);
    });
  });
});
